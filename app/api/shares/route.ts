...
import { createShareToken, hashShareToken, isSameOriginRequest, parseShareId, shareSummary } from "@/lib/reader";
...
export async function GET(request: Request): Promise<Response> {
  ...
  try {
    const savedShares = await getDb().select().from(shares).orderBy(desc(shares.id));
    return Response.json({ shares: savedShares.map(shareSummary) });
  } catch (error) {
    console.error("Unable to load shares", error);
    return Response.json({ error: "Unable to load share links" }, { status: 500 });
  }
}
...
export async function POST(request: Request): Promise<Response> {
  ...
  try {
    ...
    await db.insert(shares).values({ tokenHash: await hashShareToken(token), editionId, createdAt: new Date() });
...
export async function DELETE(request: Request): Promise<Response> {
  ...
  try {
    const shareId = parseShareId(await request.json());
...
async function requireOwner(request: Request): Promise<true | Response> {
  if (request.method !== "GET" && !isSameOriginRequest(request)) return Response.json({ error: "Cross-origin requests are not allowed" }, { status: 403 });
