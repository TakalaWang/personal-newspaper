...
3. Generate a cryptographically random 32-byte publication credential. Create `.codex-reporter/runtime.env` with mode `0600`, containing only `PAPER_URL` and `AUTOMATION_TOKEN`; leave `PAPER_URL` empty until deployment succeeds. Never print the file or credential.
...
2. Inspect existing automations. A candidate has either the previously saved automation id, or both the exact name `Codex Reporter daily edition` and exact project id. Prompt text is repairable configuration, not identity. Stop if multiple candidates exist.
