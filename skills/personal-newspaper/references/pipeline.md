...
5. Draw a private 12-column page map for each sheet. For every package record story id, band, start column, end column, occupied span, and any internal visual or cross-column interruption. This map is a reasoning artifact, not a named template and not published content.
6. Run the geometry gate on the map. With four or more packages, use at least three distinct occupied spans, offset at least one boundary between adjacent bands to create a stepped seam, and include one cross-column or compound internal interruption. Reject any persistent vertical seam that reduces the complete sheet to a left column plus right column, even if either side contains stacked stories. With three packages, reject a persistent split; use a full-width or offset band plus an unequal band, or one compound package whose internal visual changes the reading path. With fewer packages, combine the thin subject with an adjacent page unless editorial coherence requires a long-form insert.
7. Let editorial coherence determine page count, section grouping, and module count. Balance independent vertical lanes when shared grid rows would strand whitespace; reject any alignment-created cavity larger than four lines of neighboring body copy. When enough verified material exists, keep neighboring lane endings within four body lines. Remove or combine a thin page instead of padding a fixed slot.
8. In the rendered desktop preflight, measure the content-sized sheet after canonical copy injection. Target an A-series portrait silhouette with a height-to-width ratio from 1.30 through 1.55. Recompose modules or edit verified copy within its evidence when outside that range; never use `min-height`, spacer modules, filler copy, oversized padding, or an empty image well to manufacture the ratio.
